...
const HeroSection: React.FC = () => {
  ...
  return (
    <section className="relative w-full min-h-screen overflow-x-hidden bg-lightGray">
      ...
      <div className="absolute inset-0 pointer-events-none bg-[linear-gradient(#1e1e1e,transparent_0.2px),linear-gradient(90deg,#1e1e1e,transparent_0.2px)] bg-[size:60px_60px]" />
      ...
      <div className="relative z-20 flex flex-col justify-center items-center min-h-screen px-6 text-center">
        ...
        {/* Main Title */}
        <h1
          className={`font-extrabold text-smoothBlack text-5xl md:text-9xl leading-tight transition-all duration-700 ${
            mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
          }`}
          style={{ transitionDelay: '300ms' }}
        >
          We Build{' '}
          <span className="relative inline-block text-appleBlue">
            {words[currentWord]}
            <span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 animate-[pulse_2s_infinite]" />
          </span>
          <br />
          <span className="text-smoothBlack">Experiences</span>
        </h1>
